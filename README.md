...
`target="_blank"` on the form is required, not optional styling — MailerLite's endpoint doesn't redirect back to the page it was submitted from, it opens its own confirmation page. So a successful submit gives the visitor a new tab showing a plain MailerLite page, not this site; that's expected for this bare-HTML approach, not a bug. (An earlier version tried submitting into a hidden iframe to avoid that, on the assumption the endpoint would respond inline — it didn't work reliably, most likely because MailerLite blocks framing its own pages. Don't reintroduce that.) Since there's no on-page confirmation after submit, `newsletterConfirmNote` in `i18n.ts` is a small permanent line under the form telling the visitor to expect a confirmation email — always visible, not conditional on submission. Double opt-in is on in MailerLite, so that note is correct: nothing arrives until the visitor clicks the link in that email.
