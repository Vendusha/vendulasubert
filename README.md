...
**Browser gotcha, easy to reintroduce by accident:** any `<img>` styled with CSS `aspect-ratio` but no explicit `height` will silently ignore that `aspect-ratio` and fall back to whatever height its `width`/`height` HTML attributes imply — Astro's `<Picture>`/`<Image>` always sets those attributes from the source photo's own uncropped proportions, so this is very easy to trip over ("why do my three cropped photos have three different heights even though they all say `aspect-ratio: 4/3`?"). The fix is just adding `height: auto` alongside `aspect-ratio` — see `.column-card img` on the Home page. If a new `aspect-ratio` crop elsewhere on the site ever looks like it's not taking effect, check for this first.
...
The footer form (`src/components/Newsletter.astro`) is a plain HTML POST to MailerLite (`NEWSLETTER_FORM_ENDPOINT` in `src/lib/i18n.ts`) — deliberately not MailerLite's JavaScript embed, so there's no tracking script on the site. The same form also appears at the end of every blog post; the two instances are fully independent (separate iframe, separate success message — see below).

It submits into a hidden `<iframe>` (via the form's `target` attribute), so the visitor never leaves the page. A small vanilla-JS snippet in the same component — ours, not MailerLite's, no tracking — listens for that iframe's `load` event and swaps the form for a success message (`newsletterSuccess` in `i18n.ts`) when it fires. Because the iframe's response is cross-origin, the script can't actually read whether MailerLite accepted the address; "the iframe finished loading" is the only signal available, so a genuinely malformed submission (rare, given the browser's own `type="email"` validation) would still show success. Acceptable tradeoff for a newsletter signup. Without JavaScript, the form still submits and still works — it just won't visually swap to the success message, so the permanent `newsletterConfirmNote` line right under the form is what tells a no-JS visitor to expect a confirmation email.

Double opt-in is on in MailerLite, so both messages are correct either way: nothing arrives until the visitor clicks the link in that email.

The field name `fields[email]` and the hidden `ml-submit` field are MailerLite's required names, not arbitrary — don't rename them. If the form ever moves to a different MailerLite form/account, update `NEWSLETTER_FORM_ENDPOINT` — the account and form IDs are baked into that URL's path, no separate hidden ID fields needed.
